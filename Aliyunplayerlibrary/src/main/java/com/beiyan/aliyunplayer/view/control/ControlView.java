package com.beiyan.aliyunplayer.view.control;
...
import com.beiyan.aliyunplayer.bean.DotBean;
import com.beiyan.aliyunplayer.constants.GlobalPlayerConfig;
import com.beiyan.aliyunplayer.quality.QualityItem;
import com.beiyan.aliyunplayer.theme.ITheme;
import com.beiyan.aliyunplayer.theme.Theme;
import com.beiyan.aliyunplayer.util.AliyunScreenMode;
import com.beiyan.aliyunplayer.util.TimeFormater;
import com.beiyan.aliyunplayer.view.dot.DotView;
import com.beiyan.aliyunplayer.view.function.AdvVideoView;
import com.beiyan.aliyunplayer.view.function.MutiSeekBarView;
import com.beiyan.aliyunplayer.view.interfaces.ViewAction;
import com.beiyan.aliyunplayer.view.speed.SpeedView;
...
/**
 * @author : C4_雍和
 * 描述 :控制条界面。包括了顶部的标题栏，底部 的控制栏，锁屏按钮等等。是界面的主要组成部分。
 * 主要功能 :
 * 维护人员 : C4_雍和
 * date : 20-11-2 下午5:47
 */
public class ControlView extends RelativeLayout implements ViewAction, ITheme {
    ...
    //跳过
    private TextView oralDefenseSkip;
    //进度条
    ...
    //播放倍速
    private Button mLargeChangeVideoRateBtn;
    //更多弹窗按钮
    private ImageView mTitleMore;
    //下载进度
    private TextView mTitleProgressText;
    //这些是小屏时显示的
    //底部控制栏
    ...
    //速率按钮点击监听
    private OnVideoRateBtnClickListener mOnVideoRateBtnClickListener;
    // 显示更多
    ...
    private SpeedView.SpeedValue speedValue = SpeedView.SpeedValue.Normal;
    /**
     * 下载状态
     * 0未下载
     * 1正在下载
    ...
    public void updateTopRightImg(int stas) {
        downloadState = stas;
        if (downloadState == 0) {
            mTitleMore.setVisibility(View.VISIBLE);
            mTitleProgressText.setVisibility(View.GONE);
            mTitleMore.setImageResource(R.drawable.icon_donwload);
        } else if (downloadState == 1) {
            mTitleMore.setVisibility(View.GONE);
            mTitleProgressText.setVisibility(View.VISIBLE);
            mTitleMore.setImageResource(R.drawable.icon_donwload);
        } else if (downloadState == 2) {
            mTitleMore.setVisibility(View.VISIBLE);
            mTitleProgressText.setVisibility(View.GONE);
            mTitleMore.setImageResource(R.drawable.ic_delete);
    ...
    private void findAllViews() {
        ...
        mTitleProgressText = (TextView) findViewById(R.id.alivc_progress_download);
        ...
        mLargeChangeVideoRateBtn = (Button) findViewById(R.id.alivc_info_video_rate_btn);
        ...
        oralDefenseSkip = (TextView) findViewById(R.id.oral_defense_skip);
    }
    ...
    private void setViewListener() {
        ...
        mLargeChangeVideoRateBtn.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View view) {
                //点击切换分辨率 显示分辨率的对话框
                if (mOnVideoRateBtnClickListener != null) {
                    mOnVideoRateBtnClickListener.onVideoRateBtnClick(view, speedValue);
                }
            }
        });
        // 更多按钮点击监听
    ...
    private void updateShowMoreBtn() {
        if (mAliyunScreenMode == AliyunScreenMode.Full && !mInScreenCosting) {
            ...
            mTitleProgressText.setVisibility(GONE);
        } else {
            updateTopRightImg();
        }
    }
    ...
    public void updateTopRightImg() {
        updateTopRightImg(downloadState);
    }
    ...
    public AdvVideoView.IntentPlayVideo getIntentPlayVideo(int currentPosition, int seekToPosition) {
        if (mAliyunScreenMode == AliyunScreenMode.Full && mLargeMutiSeekbar != null) {
     ...
     * 当forceQuality的时候不可见。
     */
    public void updateChangeVideoRateBtn(String text, SpeedView.SpeedValue speedValue) {
        if (mLargeChangeVideoRateBtn != null) {
            this.speedValue = speedValue;
            VcPlayerLog.d(TAG, "mCurrentQuality = " + mCurrentQuality + " , isMts Source = " + isMtsSource + " , mForceQuality = " + mForceQuality);
            mLargeChangeVideoRateBtn.setText(text);
        }
    }
    ...
    public interface OnVideoRateBtnClickListener {
        /**
         * 清晰度按钮被点击
         *
         * @param v              被点击的view
         * @param currentQuality 当前清晰度
         */
        void onVideoRateBtnClick(View v, SpeedView.SpeedValue currentQuality);

        /**
         * 隐藏
         */
        void onHideVideoRateView();
    }

    public void setmOnVideoRateBtnClickListener(OnVideoRateBtnClickListener mOnVideoRateBtnClickListener) {
        this.mOnVideoRateBtnClickListener = mOnVideoRateBtnClickListener;
    }
    ...
    public TextView getOralDefenseSkip() {
        return oralDefenseSkip;
    }

    public void setDownloadProgress(int percent) {
        if (percent > 0) {
            if (mTitleMore.getVisibility() == View.VISIBLE && mTitleProgressText.getVisibility() == View.GONE) {
                updateTopRightImg(1);
            }
            mTitleProgressText.setText(String.valueOf(percent));
        }
        if (percent == 100) {
            updateTopRightImg(2);
            mTitleProgressText.setText("");
        }
    }
}
