package com.beiyan.aliyunplayer.view.gesture;
...
/**
 * @author : C4_雍和
 * 描述 :播放控手势控制。通过对view的GestureDetector事件做监听，判断水平滑动还是垂直滑动。
    ...
    private final GestureDetector.OnGestureListener mOnGestureListener = new GestureDetector.OnGestureListener() {
        ...
        public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
            ...
            if (isInHorizenalGesture) {
                 ...
                 */
                if(mIsMultiWindow){
                    if (ScreenUtilsTwo.isInLeft(mView, (int) mXDown)) {
                        isInLeftGesture = true;
                        if (mGestureListener != null) {
                            mGestureListener.onLeftVerticalDistance(e1.getY(), e2.getY());
                        }
                    } else if (ScreenUtilsTwo.isInRight(mView, (int) mXDown)) {
                    ...
                    if (ScreenUtilsTwo.isInLeft(mContext, (int) mXDown)) {
                        isInLeftGesture = true;
                        if (mGestureListener != null) {
                            mGestureListener.onLeftVerticalDistance(e1.getY(), e2.getY());
                        }
                    } else if (ScreenUtilsTwo.isInRight(mContext, (int) mXDown)) {
                        isInRightGesture = true;
                        if (mGestureListener != null) {
                            mGestureListener.onRightVerticalDistance(e1.getY(), e2.getY());
                        }
