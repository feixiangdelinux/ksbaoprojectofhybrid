...
import com.beiyan.aliyunplayer.bean.LongVideoBean;
import com.beiyan.aliyunplayer.constants.GlobalPlayerConfig;
import com.beiyan.aliyunplayer.database.LoadDbDatasListener;
import com.beiyan.aliyunplayer.database.LoadDbTvListDatasListenerr;
import com.beiyan.aliyunplayer.database.LongVideoDatabaseManager;
import com.beiyan.aliyunplayer.listener.RefreshStsCallback;
import com.beiyan.aliyunplayer.util.ThreadUtils;
import com.beiyan.aliyunplayer.util.VidStsUtil;
...
public class AliyunDownloadManager {
     ...
     * 准备下载项目
     */
    public void prepareDownload(final VidSts vidSts, String videoCode, int appID, String videoPath, String userName, int functionType) {
        if (vidSts == null || TextUtils.isEmpty(vidSts.getVid())) {
            return;
        }
        this.mVidSts = vidSts;
        ...
        jniDownloader.setOnPreparedListener(new AliMediaDownloader.OnPreparedListener() {
            @Override
            public void onPrepared(MediaInfo mediaInfo) {
                onPreparedCallback(vidSts, mediaInfo, downloadMediaInfos, null, videoCode, appID, videoPath, userName, functionType);
        ...
        AliyunDownloadMediaInfo mediaInfo = new AliyunDownloadMediaInfo();
        mediaInfo.setVidSts(vidSts);
        mediaInfo.videoCode = videoCode;
        mediaInfo.appID = appID;
        mediaInfo.functionType = functionType;
        mediaInfo.videoPath = videoPath;
    ...
    private void onPreparedCallback(VidSts vidSts, MediaInfo mediaInfo, List<AliyunDownloadMediaInfo> downloadMediaInfos, LongVideoBean longVideoBean, String videoCode, int appID, String videoPath, String userName, int functionType) {
        List<TrackInfo> trackInfos = mediaInfo.getTrackInfos();
        for (TrackInfo trackInfo : trackInfos) {
...
//                        //一个AliMediaDownloader 对应多个 AliyunDownloaderMediaInfo(同一Vid,不同清晰度)
                ...
                downloadMediaInfo.videoCode = videoCode;
                downloadMediaInfo.appID = appID;
                downloadMediaInfo.videoPath = videoPath;
                downloadMediaInfo.userName = userName;
                downloadMediaInfo.functionType = functionType;
...
//                        //一个AliMediaDownloader 对应多个 AliyunDownloaderMediaInfo(同一Vid,不同清晰度)
    ...
    public void startDownload(final AliyunDownloadMediaInfo downloadMediaInfo) {
        ...
        if (downloadMediaInfo.getStatus() == AliyunDownloadMediaInfo.Status.Complete) {
            ...
            if (file.exists()) {
                Toast.makeText(mContext.getApplicationContext(), mContext.getResources().getString(R.string.alivc_adv_video_tips), Toast.LENGTH_SHORT).show();
                return;
            }
        }
