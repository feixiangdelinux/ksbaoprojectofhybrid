...
import com.beiyan.aliyunplayer.database.DatabaseHelper;
...
public class DatabaseManager {
    ...
    public static final String CREATE_TABLE_SQL = "create table if not exists " + DatabaseManager.TABLE_NAME +
            ...
            DatabaseManager.VIDTYPE + " integer," + DatabaseManager.APPID + " integer," +
            DatabaseManager.VIDEOCODE + " text," + DatabaseManager.VIDEOPATH + " text," +
            DatabaseManager.USERNAME + " text," + DatabaseManager.FUNCTIONTYPE + " integer," +
    ...
    /**
     * videoCode='fd92ce101e73f0f73a0b582e431a7819',
     * appID=1552,
     * videoPath='考点精讲|基础知识|第一章常见症状与体征|',
     * lastProgress=0,
    ...
    public long insert(AliyunDownloadMediaInfo mediaInfo) {
        ...
        contentValues.put(VIDEOCODE, mediaInfo.getVideoCode());
        contentValues.put(APPID, mediaInfo.getAppID());
        contentValues.put(FUNCTIONTYPE, mediaInfo.getFunctionType());
        contentValues.put(VIDEOPATH, mediaInfo.getVideoPath());
        contentValues.put(USERNAME, mediaInfo.getUserName());
    ...
    private List<AliyunDownloadMediaInfo> selectAllCursorToDownloadMediaInfo(Cursor cursor) {
        ...
        while (cursor.moveToNext()) {
            ...
            mediaInfo.setUserName(cursor.getString(cursor.getColumnIndex(USERNAME)));
            mediaInfo.setVideoCode(cursor.getString(cursor.getColumnIndex(VIDEOCODE)));
            mediaInfo.setVideoPath(cursor.getString(cursor.getColumnIndex(VIDEOPATH)));
            mediaInfo.setAppID(cursor.getInt(cursor.getColumnIndex(APPID)));
            mediaInfo.setFunctionType(cursor.getInt(cursor.getColumnIndex(FUNCTIONTYPE)));
            switch (status) {
                case 0:
                    //Idle
